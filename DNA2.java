...
public class DNA2 {
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        for(int i = 0; i < sequence.length(); i++) {
            if (sequence.startsWith(STR, i)) {
                ...
                if (numOfRepeats > maxRepeats) {
                    maxRepeats = numOfRepeats;
            }
            numOfRepeats = 0;
            }
        }
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        ...
        /* WRITE YOUR CODE HERE */
        String sequence1 = profile.getSequence1(); //get DNA sequence from profile
        String sequence2 = profile.getSequence2();
        
        STR[] S1_STRs = new STR[allSTRs.length]; //create two arrays to store STR objects 
        STR[] S2_STRs = new STR[allSTRs.length];
        //for loop to iterate each element in array
        for(int i = 0; i < allSTRs.length; i++) {
           String STR = allSTRs[i]; //gets i-th element from array and assigns it to STR
           STR s1 = findSTRInSequence(sequence1, STR); //call findSTRinSequence to find lonest repeats in sequences
           STR s2 = findSTRInSequence(sequence2, STR);
           S1_STRs[i] = s1; //create STR objects and store them in arrays 
           S2_STRs[i] = s2;
        }
        profile.setS1_STRs(S1_STRs); //set arrays in profile
   ...
   public boolean identicalSTRs ( STR[] s1, STR[] s2 ) {
       ...
       for(int i = 0; i < s1.length; i++) {
           if (s1[i].getRepeats() != s2[i].getRepeats()) {
               return false;
           }
       }
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        /* WRITE YOUR CODE HERE */
        ArrayList<Profile> matchingProfiles = new ArrayList<>(); //initializes ArrayList
        for (Profile profile : database) {
            STR[] profileS1_STRs = profile.getS1_STRs();
            boolean matchFound = false;
    
            if (identicalSTRs(profileS1_STRs, unknownProfileS1_STRs)) {
                matchFound = true;
            }
    ...
    public ArrayList<Profile> findPossibleParents ( STR[] S1_STRs, STR[] S2_STRs ) {
        ...
        ArrayList<Profile> possibleParent1 = new ArrayList<>();
         ...
         for ( int i = 0; i < database.length; i++ ) {
            //green checkmark is parent 2, yellow checkmark is parent 1
